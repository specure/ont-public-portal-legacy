        ...
        thread.onStateEnter(TestState.UP, function () {
            ...
            if (_bytesPerSecsPretest.length > 0) {
                ...
                if (navigator.userAgent.match(/Firefox\/82\.0.*/)) {
                    ...
                    var resultDlBitPerSec = results.speed / 4; //
                    var alternativeChunkSizes = calculateChunkSizes([resultDlBitPerSec / 8], _rmbtTestConfig.uploadThreadsLimitsMbit, true);
        ...
        var loop = function loop() {
            uploadChunks(thread, n, chunkSize, function (msg) {
                ...
                if (now - startTime > durationMs) {
                    ...
                    var newChunkSize = Math.min(chunkSize * 2, maxChunkSize);
 ...
 * @returns Object
 */
var RMBTControlServerCommunication = function RMBTControlServerCommunication(rmbtTestConfig, options, testServerConfig) {
    ...
    return {
        ...
        submitResults: function submitResults(json_data, onsuccess, onerror) {
            ...
            var test_speed_upload = json_data.test_speed_upload;
            var test_speed_download = json_data.test_speed_download;

            if (
                !test_speed_upload
                || isNaN(test_speed_upload)
                || test_speed_upload < 0
                || !test_speed_download
                || isNaN(test_speed_download)
                || test_speed_download < 0
            ) {
                onerror()
                return
            }
...
var RMBTTestConfig = function () {
    ...
    function RMBTTestConfig(language, controlProxy, wsPath, params = {}) {
        if (params.hasOwnProperty('DOWNLOAD')) {
          this.downloadThreadsLimitsMbit = params.DOWNLOAD;
        }
        if (params.hasOwnProperty('UPLOAD')) {
          this.uploadThreadsLimitsMbit = params.UPLOAD;
        }
